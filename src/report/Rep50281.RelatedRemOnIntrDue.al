report 50281 "Related Rem. On Intr. Due"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    DefaultRenderingLayout = LayoutName;
    Caption = 'Relatedparty Interest Due Reminder';
    ...
    dataset
    {
        dataitem("RelatedParty Loan"; "RelatedParty Loan")
        {
            ...
            begin
                if FunderNo <> '' then
                    "RelatedParty Loan".SetRange("No.", FunderNo);
                "RelatedParty Loan".SetRange(Status, Status::Approved);
                "RelatedParty Loan".SetFilter(MaturityDate, '>%1', Today);
                GeneralSetup.Get(0)
            end;
            ...
            begin
                ...
                */
                _remainingDays := 0;
                if "RelatedParty Loan".PeriodicPaymentOfInterest = "RelatedParty Loan".PeriodicPaymentOfInterest::Monthly then
                    _remainingDays := TrsryCU.GetDaysUntilMonthEnd();
                if "RelatedParty Loan".PeriodicPaymentOfInterest = "RelatedParty Loan".PeriodicPaymentOfInterest::Quarterly then
                    _remainingDays := TrsryCU.GetDaysUntilQuarterEnd();
                if "RelatedParty Loan".PeriodicPaymentOfInterest = "RelatedParty Loan".PeriodicPaymentOfInterest::Biannually then
                    _remainingDays := TrsryCU.GetDaysUntilBiAnnualEnd();
                if "RelatedParty Loan".PeriodicPaymentOfInterest = "RelatedParty Loan".PeriodicPaymentOfInterest::Annually then
                ...
                if true then begin //(Today - "Funder Loan".MaturityDate) = GeneralSetup."Placemnt. Matur Rem. Time"
                    ...
                    // RelatedPartyLoanTbl.SetRange("No.", FunderNo);
                    // if not RelatedPartyLoanTbl.Find('-') then
                    //     Error('Funder Loan not %1 found', FunderNo);
                    RelatedPartyLoanTbl := "RelatedParty Loan";

                    _fNo := RelatedPartyLoanTbl."No.";
                    placementDate := RelatedPartyLoanTbl.PlacementDate;
                    maturityDate := RelatedPartyLoanTbl.MaturityDate;
                    dateDiff := (maturityDate - placementDate);
                    endYearDate := CALCDATE('CY', Today);
                    remainingDays := endYearDate - RelatedPartyLoanTbl.PlacementDate;
                    ...
                    // TrsyMgt.GetInterestRate(RelatedPartyLoanTbl."No.", 'RELATEDPARTY_REPORT');
                    /* 
                    if (RelatedPartyLoanTbl.InterestRateType = RelatedPartyLoanTbl.InterestRateType::"Fixed Rate") then
                        _interestRate_Active := RelatedPartyLoanTbl.InterestRate;
                    if (RelatedPartyLoanTbl.InterestRateType = RelatedPartyLoanTbl.InterestRateType::"Floating Rate") then
                        _interestRate_Active := (RelatedPartyLoanTbl."Reference Rate" + RelatedPartyLoanTbl.Margin);
                    // if _interestRate_Active = 0 then
                    //     Error('Interest Rate is Zero');
                    */

                    _withHoldingTax_Percent := RelatedPartyLoanTbl.Withldtax;
                    _withHoldingTax_Amnt := 0;

                    RelatedPartyLoanTbl.CalcFields(OutstandingAmntDisbLCY);
                    _principle := RelatedPartyLoanTbl.OutstandingAmntDisbLCY;

                    // Loan.Reset();
                    // Loan.DeleteAll();
                    if RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal = RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal::Monthly then begin
                        ...
                        for monthCounter := 0 to NoOfMonths do begin
                            ...
                            _interestRate_Active := TrsyMgt.GetInterestRateSchedule(RelatedPartyLoanTbl."No.", _currentMonthInLoop, 'RELATEDPARTY_REPORT');


                            if RelatedPartyLoanTbl.InterestMethod = RelatedPartyLoanTbl.InterestMethod::"30/360" then begin
                                monthlyInterest := ((_interestRate_Active / 100) * _principle) * (30 / 360);
                            end else if RelatedPartyLoanTbl.InterestMethod = RelatedPartyLoanTbl.InterestMethod::"Actual/360" then begin
                                monthlyInterest := ((_interestRate_Active / 100) * _principle) * (DaysInMonth / 360);
                            end else if RelatedPartyLoanTbl.InterestMethod = RelatedPartyLoanTbl.InterestMethod::"Actual/364" then begin
                                monthlyInterest := ((_interestRate_Active / 100) * _principle) * (DaysInMonth / 364);
                            end else if RelatedPartyLoanTbl.InterestMethod = RelatedPartyLoanTbl.InterestMethod::"Actual/365" then begin
                    ...
                    if RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal = RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal::Quarterly then begin
                        //12
                        //No of days in that month
                        ...
                        for QuarterCounter := 1 to NoOfQuarter do begin
                            ...
                            _interestRate_Active := TrsyMgt.GetInterestRateSchedule(RelatedPartyLoanTbl."No.", _currentQuarterInLoop, 'RELATEDPARTY_REPORT');
                    ...
                    if RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal = RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal::Biannually then begin
                        //12
                        //No of days in that month
                        ...
                        for BiannCounter := 0 to NoOfBiann do begin
                            ...
                            //Get quarter date. - sub the current date for days.
                            //Add to the next quarter
                            _interestRate_Active := TrsyMgt.GetInterestRateSchedule(RelatedPartyLoanTbl."No.", _currentBiannInLoop, 'RELATEDPARTY_REPORT');
                    ...
                    if RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal = RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal::Annually then begin
                        //12
                        //No of days in that month
                        ...
                        for AnnualCounter := 0 to NoOfAnnual do begin
                            ...
                            _interestRate_Active := TrsyMgt.GetInterestRateSchedule(RelatedPartyLoanTbl."No.", _currentAnnualInLoop, 'RELATEDPARTY_REPORT');
                    ...
                    if RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal = RelatedPartyLoanTbl.PeriodicPaymentOfPrincipal::"Total at Due Date" then begin
                        _outstandingAmount := _principle;
                        _amortization := _principle;
                        _totalPayment := _principle;
                        _currentAnnualInLoop := maturityDate;
                    ...
                    Loan.Reset();
                    Loan.SetRange(LoanNo, "RelatedParty Loan"."No.");
                    ...
                    DimensionValue.SetRange(DimensionValue.Code, RelatedParty."Shortcut Dimension 1 Code");
                    if DimensionValue.Find('-') then
                        BranchName := DimensionValue.Name;

                    Banks.Reset();
                    Banks.SetRange(BankCode, RelatedParty."Bank Code");
                    if Banks.Find('-') then
                        BankName := RelatedParty."Bank Name";
                    // BankName := Banks.Name;

                    BankBranch.Reset();
                    BankBranch.SetRange(BranchCode, RelatedParty."Bank Branch");
    ...
    rendering
    {
        ...
        {
            Type = RDLC;
            LayoutFile = './reports/relatedreminderonintdue.rdlc';
        }
    }
