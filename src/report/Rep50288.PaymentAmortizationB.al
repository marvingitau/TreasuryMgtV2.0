report 50288 "Payment Amortization B"
{
    ...
    rendering
    {
        ...
        {
            Type = RDLC;
            LayoutFile = './reports/PaymentAmortizationB.rdlc';
        }
    }
    trigger OnPreReport()
    var
    ...
    begin
        ...
        _dueDateInfluence := FunderLoanTbl.EnableDynamicPeriod_Payment;
        _skipWeekendInfluence := FunderLoanTbl.EnableWeekDayReporting_Payment;
        ...
        if FunderLoanTbl.PeriodicPaymentOfPrincipal = FunderLoanTbl.PeriodicPaymentOfPrincipal::Monthly then begin
            ...
            if ((_dueDateInfluence = true) and (_skipWeekendInfluence = true)) then begin
                ...
                for monthCounter := 0 to NoOfMonths do begin
                    ...
                    if (monthCounter = 0) and (monthCounter <> NoOfMonths) then begin
                        ...
                        // _floatingOutstandingValue:=_floatingOutstandingValue-
                    ...
                    _GlobaloutstandingAmount := _outstandingAmount;

                    if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/360" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (30 / 360);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/360" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInMonth / 360);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/364" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInMonth / 364);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInMonth / 365);
                    end
                    else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (30 / 365);
                    end;

                    _totalPayment := _dailyPrincipal + monthlyInterest;
                    ...
                    if (dueDate <> 0D) and (dueDate = _currentMonthInLoop) then begin
                        ...
                        FirstDueAccumulator.TotalPayment := _totalPayment;
                        ...
                        Loan.TotalPayment := _totalPayment;
                        Loan.Insert();
            ...
            */
            ...
            if ((_dueDateInfluence = false) and (_skipWeekendInfluence = false)) then begin
                //No of days in that month
                NoOfMonths := MonthsBetween(dueDate, maturityDate);
                _dailyPrincipal := _principle / (NoOfMonths + 1);

                for monthCounter := 0 to NoOfMonths do begin
                    _currentMonthInLoop := 0D;
                    if (monthCounter = 0) then begin
                        _currentMonthInLoop := CalcDate('<CM>', dueDate);
                    end
                    else if (monthCounter = NoOfMonths) then begin
                        ...
                        _currentMonthInLoop := CalcDate('<CM>', CalcDate('<' + Format(monthCounter) + 'M>', dueDate));
                        _outstandingAmount := _principle - (_dailyPrincipal * (monthCounter + 1));
                    end;
                    ...
                    if (monthCounter = 0) then begin
                        //Start Date
                        if CalcDate('<CM>', dueDate) <> dueDate then // Is placement End of Month, if not, on the diff add 1
                            DaysInMonth := CalcDate('<CM>', dueDate) - dueDate + 1
                        else
                            DaysInMonth := CalcDate('<CM>', dueDate) - dueDate + 0;
                    ...
                    if (dueDate <> 0D) and (dueDate > _currentMonthInLoop) and (dueDate <> _currentMonthInLoop) then begin
                        ...
                        Loan.LoopCount := monthCounter;
                        Loan.Amortization := _dailyPrincipal + 0;
                        Loan.InterestRate := _interestRate_Active;
                        Loan.TotalPayment := _totalPayment + _sumTotalPayment;
        ...
        if FunderLoanTbl.PeriodicPaymentOfPrincipal = FunderLoanTbl.PeriodicPaymentOfPrincipal::Quarterly then begin
            ...
            if ((_dueDateInfluence = true) and (_skipWeekendInfluence = true)) then begin
                ...
                for QuarterCounter := 0 to NoOfQuarter do begin
                    ...
                    if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/360" then begin
                        ...
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInQuarter / 360);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/364" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInQuarter / 364);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInQuarter / 365);
                    end
                    else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (30 / 365);
                    end;
            ...
            */
            ...
            if ((_dueDateInfluence = false) and (_skipWeekendInfluence = false)) then begin
                ...
                NoOfQuarter := QuartersBetween(dueDate, maturityDate) + 0; // No of Quarters
                                                                           // StatingQuarterEndDate := GetClosestQuarterEndDate(dueDate);
                StatingQuarterStartDate := GetClosestQuarterStartDate(dueDate);
                StatingQuarterEndDate := GetEndOfQuarter(dueDate);
                _dailyPrincipal := _principle / (NoOfQuarter + 1);
                ...
                for QuarterCounter := 0 to NoOfQuarter do begin
                    ...
                    if QuarterCounter = 0 then begin
                        _currentQuarterInLoop := GetEndOfQuarter(dueDate);
                        if _currentQuarterInLoop <> dueDate then
                            DaysInQuarter := (_currentQuarterInLoop - dueDate) + 1
                        else
        ...
        if FunderLoanTbl.PeriodicPaymentOfPrincipal = FunderLoanTbl.PeriodicPaymentOfPrincipal::Biannually then begin
            ...
            if ((_dueDateInfluence = true) and (_skipWeekendInfluence = true)) then begin
                ...
                for BiannCounter := 0 to NoOfBiann do begin
                    ...
                    if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/360" then begin
                        ...
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInBiann / 360);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/364" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInBiann / 364);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInBiann / 365);
                    end
                    else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (30 / 365);
                    end;
            ...
            */
            if ((_dueDateInfluence = false) and (_skipWeekendInfluence = false)) then begin
                NoOfBiann := CountBiannualPeriodsCrossed(dueDate, maturityDate) + 0; // No of Bianuals
                StatingBiannEndDate := GetBiannualEndDate(dueDate);
                StatingBiannStartDate := GetClosestBiannualStartDate(dueDate);
                _dailyPrincipal := _principle / (NoOfBiann + 1);
                ...
                for BiannCounter := 0 to NoOfBiann do begin
                    ...
                    if BiannCounter = 0 then begin
                        _currentBiannInLoop := GetEndOfBiannual(dueDate);
                        if GetEndOfBiannual(dueDate) <> dueDate then
                            DaysInBiann := _currentBiannInLoop - dueDate + 1
                        else
                            DaysInBiann := _currentBiannInLoop - dueDate;
                        _outstandingAmount := _principle - _dailyPrincipal;
                    end
        ...
        if FunderLoanTbl.PeriodicPaymentOfPrincipal = FunderLoanTbl.PeriodicPaymentOfPrincipal::Annually then begin
            ...
            if ((_dueDateInfluence = true) and (_skipWeekendInfluence = true)) then begin
                ...
                for AnnualCounter := 0 to NoOfAnnual do begin
                    ...
                    if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/360" then begin
                        ...
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInAnnual / 360);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/364" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInAnnual / 364);
                    end else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"Actual/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (DaysInAnnual / 365);
                    end
                    else if FunderLoanTbl.InterestMethod = FunderLoanTbl.InterestMethod::"30/365" then begin
                        monthlyInterest := ((_interestRate_Active / 100) * _GlobaloutstandingAmount) * (30 / 365);
                    end;
            ...
            */
            if ((_dueDateInfluence = false) and (_skipWeekendInfluence = false)) then begin
                ...
                StatingAnnualEndDate := GetClosestAnnualEndDate(dueDate);
                _dailyPrincipal := _principle / NoOfAnnual;
                for AnnualCounter := 1 to NoOfAnnual do begin
                    _currentAnnualInLoop := 0D;
                    DaysInAnnual := 0;
                    // if AnnualCounter = 0 then begin
                    //     _currentAnnualInLoop := GetStartOfYear(dueDate);
                    //     DaysInAnnual := _currentAnnualInLoop - dueDate + 1;
                    //     _outstandingAmount := _principle;
                    // end
                    // else
                    if AnnualCounter = 1 then begin
                        _currentAnnualInLoop := GetEndOfYear(dueDate);
                        DaysInAnnual := _currentAnnualInLoop - dueDate;
                        _outstandingAmount := _principle - _dailyPrincipal;
                    end
                    else if AnnualCounter = NoOfAnnual then begin
